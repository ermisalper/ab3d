...
def fix_obj_for_printing(input_path, output_path=None, target_height_mm=75.0):
    ...
    if output_path is None:
        ...
        output_path = f"{stem}.fixed{suffix or '.obj'}"

    if os.path.abspath(output_path) == os.path.abspath(input_path):
        raise ValueError("Refusing to overwrite the source OBJ; choose a separate output path.")
...
def main():
    ...
    parser.add_argument("--output", default=None,
                        help="Output OBJ file path (default: <input>.fixed.obj; source is never overwritten)")
